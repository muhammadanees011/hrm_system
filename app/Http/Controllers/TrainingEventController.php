...
class TrainingEventController extends Controller
{
    public function index()
    {
        if (\Auth::user()->can('Manage Event')) {
            ...
            return view('trainingevent.index', compact('arrEvents', 'employees','current_month_event','events'));
        } else {
    ...
    public function create()
    {
        if (\Auth::user()->can('Create Event')) {
            ...
            return view('trainingevent.create', compact('employees', 'branch', 'departments'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('Create Event')) {
            ...
            return redirect()->route('trainingevent.index')->with('success', __('Event Successfully Created.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function edit($event)
    {
        ...
        // if (\Auth::user()->can('Edit Event')) {
            ...
            // if ($event->created_by == Auth::user()->creatorId()) {
                $employees = Employee::where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');
                return view('trainingevent.edit', compact('event', 'employees'));
            // } else {
            //     return response()->json(['error' => __('Permission denied.')], 401);
            // }
        // } else {
    ...
    public function destroy(LocalEvent $event)
    {
        if (\Auth::user()->can('Delete Event')) {
            if ($event->created_by == \Auth::user()->creatorId()) {
                ...
                return redirect()->route('trainingevent.index')->with('success', __('Event successfully deleted.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
