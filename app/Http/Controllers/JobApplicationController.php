...
class JobApplicationController extends Controller
{
    ...
    public function show($ids)
    {
        if (\Auth::user()->can('Show Job Application')) {
            ...
            $applicationAnswers = JobApplicationAnswer::where('job_application_id', $id)->with(['question', 'question.options'])->get();

            $notes = JobApplicationNote::where('application_id', $id)->get();

            $stages = JobStage::where('created_by', \Auth::user()->creatorId())->get();

            return view('jobApplication.show', compact('jobApplication', 'notes', 'stages', 'applicationAnswers'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
