            ...
            <div class="card-body">
                <div class="row" id="branching-questions">
                    <div class="form-group col-md-12">
                        <input type="hidden" name="question_template_id" value="{{$questionTemplate->id}}" class="form-control" required>
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        <input type="button" value="{{ __('Add Question') }}" id="add-question" class="add-question btn btn-secondary">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        let questionEditCounter = {{ isset($questionTemplate) ? $questionTemplate->questions->count() : 0 }};

        // Function to add a new question to the form
        function addQuestion(i) {
            const questionDiv = $(`<div class="row mb-3" data-question-counter="${questionCounter}">`);
            ...
            questionDiv.append(`
                    <input type="hidden" name="${baseName}[question_id]" class="form-control" required>
            `);
            ...
            if (questionCounter < questionEditCounter) {
                ...
                let existingQuestion = @json($questionTemplate->questions);
                existingQuestion=i!=null ? existingQuestion[i]:existingQuestion;
                ...
                questionDiv.find('input[name$="[question_id]"]').val(existingQuestion.id);
                ...
                if (existingQuestion.type === 'radio') {
                    ...
                    existingQuestion.options.forEach((option, index) => {
                        if (index > 1) {
                            ...
                            const optionBaseName = `questions[${questionCounter}][options][]`;
                            optionsDiv.append('.add-option-section').before(`
                                <div class="input-group mt-2 ">
                                    <input type="text" name="${optionBaseName}" class="form-control custom-width" placeholder="New Option" value="${option.option_text}">
                            ...
                            // optionsDiv.append(`input[name$="[options][]"]`).eq(index).val(option.option_text);
                        }
                    });
                }                
            }
        ...
        $('#branching-questions').on('change', '[name$="[type]"]', function() {
            const wordCountSection = $(this).closest('.row').find('.wordCountSection');
        ...
        $('#branching-questions').on('click', '.add-option', function() {
            ...
            const baseName = optionsDiv.closest('.row').find('select[name$="[type]"]').attr('name').replace('[type]', '');
