                ...
                <div class="col text-end">
                    <ul class="list-inline mb-0">
                        ...
                        <li class="list-inline-item">
                            ...
                            <a href="#" class="bs-pass-para" data-bs-toggle="tooltip" title="" data-bs-original-title="Archive" aria-label="Delete">
                        ...
                        <li class="list-inline-item">
                            ...
                            <a href="#" class="bs-pass-para" data-bs-toggle="tooltip" title="" data-bs-original-title="Delete" aria-label="Delete"><span class="badge bg-danger p-2 px-3 rounded">{{ __('Delete') }}</span></a>
                            {!! Form::close() !!}
                        </li>
                        @endif
                        @endcan
                ...
                <div class="d-flex align-items-center" data-toggle="tooltip" data-placement="right" data-title="2 hrs ago" data-original-title="" title="">
                    <div>
                        ...
                        <a href="{{ !empty($jobApplication->profile) ?($profiles . $jobApplication->profile) : $logo."avatar.png" }}" target="_blank" class="avatar rounded-circle avatar-sm">
                            <img src="{{ !empty($jobApplication->profile) ? ($profiles . $jobApplication->profile) : $logo."avatar.png" }}" class="img-fluid rounded-circle" style="width: 50px">
                        </a>
                    </div>
    ...
    <div class="card">
            ...
            <div class="row">
                ...
                <div class="col text-end">
                    <div class="col-12 text-end">
                        <a href="#" data-url="{{ route('job.on.board.create', $jobApplication->id) }}" data-ajax-popup="true" class="btn-sm btn btn-primary" data-title="{{ __('Add to Job OnBoard') }}" data-bs-original-title="{{ __('Add to Job OnBoard') }}">
        ...
        <div class="card-header">
            <dl class="row">
                ...
                @can('DOB')
                @if (!empty($jobApplication->dob) && \Auth::user()->can('DOB'))
                <dt class="col-sm-3"><span class="h6 text-sm mb-0">{{ __('DOB') }}</span></dt>
                <dd class="col-sm-8"><span class="text-sm">{{ \Auth::user()->dateFormat($jobApplication->dob) }}</span></dd>
                @endif
                @endcan

                @can('Gender')
                @if (!empty($jobApplication->gender)&& \Auth::user()->can('Gender'))
                ...
                @can('Address')
                @if (!empty($jobApplication->address) && \Auth::user()->can('Address'))
                ...
                @can('City')
                @if (!empty($jobApplication->city)&& \Auth::user()->can('City'))
                ...
                @can('State')
                @if (!empty($jobApplication->state)&& \Auth::user()->can('State'))
                ...
                @can('Country')
                @if (!empty($jobApplication->country) && \Auth::user()->can('Country'))
                ...
                @can('ZipCode')
                @if (!empty($jobApplication->zip_code)&& \Auth::user()->can('ZipCode'))
                ...
                <dd class="col-sm-8"><span class="text-sm">{{ !empty($jobApplication->jobs) ? $jobApplication->jobs->title : '-' }}</span>
                </dd>
                @endcan

                @can('Applied at')
                <dt class="col-sm-3"><span class="h6 text-sm mb-0">{{ __('Applied at') }}</span></dt>
                <dd class="col-sm-8"><span class="text-sm">{{ \Auth::user()->dateFormat($jobApplication->created_at) }}</span>
                </dd>
                @endcan
                ...
                <dd class="col-sm-8">
                    ...
                    <span class="text-sm action-btn bg-primary ms-2">
                        <a href="{{ $resumes . '/' . $jobApplication->resume }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('download') }}" download=""><i class="ti ti-download text-white"></i></a>
                    </span>
        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col text-end">
                    ...
                    <a data-url="{{ route('interview-schedule.create', $jobApplication->id) }}" data-size="lg" class="btn-sm btn btn-primary" data-ajax-popup="true" data-title="{{ __('Create New Interview Schedule') }}" style="color: white">
                ...
                <div class="list-group-item px-0">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto text-end">
                            ...
                            {!! Form::open(['method' => 'DELETE', 'route' => ['job.application.note.destroy', $note->id], 'id' => 'delete-form-' . $note->id]) !!}
                            <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para" data-bs-toggle="tooltip" title="" data-bs-original-title="Delete" aria-label="Delete"><i class="ti ti-trash text-white"></i></a>
                            </form>
...
<div class="col-md-7">
    <div class="card card-fluid">
        <div class="card-header">
            <div class="row">
                <div class="col">
                    <h6 class="text-muted">{{ __('Answer of Question Template') }}</h6>
                </div>
            </div>
        </div>
        <div class="card-body">
            <div class="list-group list-group-flush mb-4">
                <div class="list-group-item px-0">
                    @if(count($applicationAnswers) > 0)
                    @forEach($applicationAnswers as $questionAnswer)
                    <div class="row align-items-center mb-2">
                        <h5>{{$questionAnswer->question->name}}</h5>
                        <p class="card-text text-muted mb-0">
                            {{$questionAnswer->question->type == 'radio' ?  \App\Models\QuestionOption::getOptionTextById($questionAnswer->answer) : $questionAnswer->answer}} 
                        </p>
                    </div>
                    @endforeach
                    @else
