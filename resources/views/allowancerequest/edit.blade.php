...
{{ Form::model($allowancerequest, ['route' => ['allowancerequest.update', $allowancerequest->id], 'method' => 'PUT']) }}
    ...
    <div class="row">
        ...
        <div class="form-group col-md-12">
            ...
            <select name="status" class="form-control  select2" id="status">
                <option value="Pending"  @if($allowancerequest->status=='Pending') selected @endif>{{ __('Pending') }}</option>
                <option value="Approved" @if($allowancerequest->status=='Approved') selected @endif>{{ __('Approved') }}</option>
                <option value="Rejected" @if($allowancerequest->status=='Rejected') selected @endif>{{ __('Rejected') }}</option>
            </select>
        </div>
        <div class="form-group">
