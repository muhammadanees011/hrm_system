...
@if (\Auth::user()->type == 'employee')
    ...
    <div class="col-xxl-12">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row align-items-center py-0 gap-4 marked-attandance">
                    <div class="col-auto p-3" style="background: rgba(88,78,210,0.2);border-radius: 10px;">
                        <svg xmlns="http://www.w3.org/2000/svg" class="text-success" width="96" height="96"
                    ...
                    <div class="col-10">
                        ...
                        @if(!empty($flexiTime))
                            <p class="text-warning p-1">Your request time of {{$flexiTime->hours}} hours for today will be
                                adjusted when you clockout.</p>
                        @endif
                                ...
                                @if (!empty($employeeAttendance) && $employeeAttendance->clock_out == '00:00:00')
                                    <div class="d-flex justify-content-between">
                                        ...
                                        @if (empty($employeeAttendance->break_start) || $employeeAttendance->break_start === '00:00:00')
                                            ...
                                            <button type="submit" value="start" name="break"
                                                class="btn btn-warning text-nowrap text-truncate">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                    stroke-linejoin="round"
                                        ...
                                        @elseif (!empty($employeeAttendance->break_start) && ($employeeAttendance->break_end === '00:00:00' || empty($employeeAttendance->break_end)))
                                            ...
                                            <button type="submit" value="end" name="break"
                                                class="btn btn-warning">{{ __('END BREAK') }}</button>
                                        @endif
                    ...
                    @foreach ($leaves as $leave)
                        <li class="mb-2">
                            <h6 class="mb-0 text-primary">
                                {{ $leave->leaveType->title}}|{{ ucfirst(str_replace('_', ' ', $leave->leave_duration)) }}
                            </h6>
                            <span class="text-dark"> <b>Start Date : </b> {{\Auth::user()->dateFormat($leave->start_date)}}
                            </span>
    ...
    @else
        <script>
            function startTour() {
                ...
                const markedAtt = document.querySelector('.marked-attandance');
                const header = document.querySelector('.header-wrapper');
                const navbar = document.querySelector('.dash-navbar');
                const navbarItems = navbar.querySelectorAll('.dash-item')
                tour.setOptions({
                    steps: [
                        {
                            intro: '<h6 style="margin-bottom: 2px;" class="text-center text-warning">Welcome to your HRM dashboard!</h6><br/><p style="font-size: 14px; text-align:center;">This is where you can manage tasks, notifications, and more.</p><br/><p style="font-size: 14px;margin-left: 24px;" class="badge text-warning">Let’s take a quick tour of the panel features <i class="ti ti-arrow-right"></i></p>',
                            highlightClass: 'welcome-wizard-employee',
                            title: 'Welcome to HRM Pro Tour'
                        },
                        {
                            intro: "Let me walk you through the sections here.",
                            title: 'Get ready for HRM Pro Tour'
                        },
                        {
                            element: header,
                            intro: "This is the main header where you can view your login status, notifications, messages, and change your app language.",
                            title: 'Main Header'
                        },
                        {
                            element: navbar,
                            intro: 'This is the main sidebar where you can navigate to different modules of the app.',
                            title: 'Main Sidebar'
                        },
                        {
                            element: markedAtt,
                            intro: 'Here you can mark your attandance whenver you logged into the platform.',
                            position: 'top',
                            title: 'Marking Attandance'
                        },
                        {
                            element: '#clock_in',
                            intro: "By Clicking here, You can clock in when you're logged-in.",
                            disableInteraction: true,
                            title: 'Clock In Button'
                        },
                        {
                            element: '#clock_out',
                            intro: 'Click this button to clock out before you leave.',
                            title: 'Clock Out Button'
                        },
                        {
                            element: '#event_calendar',
                            intro: 'Here you can view events on the calendar and see a list of upcoming events.',
                            title: 'Events Calendar'
                        },
                        {
                            intro: 'Now, let’s explore some other modules in the app.',
                            title: 'Head to Next Step'
                        },
                        {
                            element: navbarItems[2],
                            intro: 'Let’s meet your team Or Click on Meet Team',
                            nextLabel: 'Let\'s Go',
                ...
                tour.onbeforechange(function (targetElement) {
                    ...
                    if (currentStep === 10) {  // Step 3 is index 2, since steps are 0-indexed
                        ...
                        if (!navbarItems[2].classList.contains('clicked')) {
                            // Stop the tour from continuing if the element isn't clicked
                            window.location.href = '/meet-team'
                            return false;  // Prevent the tour from advancing
                        }
                    }
