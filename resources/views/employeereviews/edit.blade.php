...
{{ Form::model($employeereview, ['route' => ['employeereviews.update', $employeereview->id], 'method' => 'PUT']) }}
<div class="modal-body " id="reviewModal">
    ...
    <div class="row">
        ...
        <div class="form-group col-md-12">
            ...
            <div class="d-flex justify-content-between">
                <div class="form-check form-check">
                    <input class="form-check-input" type="radio" id="all" value="all" name="who_can_review"
                    @if ($employeereview->who_can_review == 'all') checked @endif>
                    <label class="form-check-label" for="all">
                        {{ __('All') }}
                    </label>
                </div>
                <div class="form-check form-check">
                    <input class="form-check-input" type="radio" id="employee" value="employee" name="who_can_review"
                    @if ($employeereview->who_can_review == 'employee') checked @endif>
                    <label class="form-check-label" for="employee">
                        {{ __('Employee') }}
                    </label>
                </div>
                <div class="form-check form-check">
                    <input class="form-check-input" type="radio" id="manager" value="manager" name="who_can_review"
                    @if ($employeereview->who_can_review == 'manager') checked @endif>
                    <label class="form-check-label" for="manager">
                        {{ __('Manager') }}
                    </label>
                </div>
                <div class="form-check form-check">
                    <input class="form-check-input" type="radio" id="hr" value="hr" name="who_can_review"
                    @if ($employeereview->who_can_review == 'hr') checked @endif>
                    <label class="form-check-label" for="hr">
                        {{ __('HR') }}
                    </label>
                </div>
                <div class="form-check form-check">
                    <input class="form-check-input" type="radio" id="select" value="select" name="who_can_review"
                    @if ($employeereview->who_can_review == 'select') checked @endif>
        ...
        @if($employeereview->who_can_review != 'select')
        ...
        <div class="form-group col-12 switch-width select-reviewer hide">
            {{ Form::label('reviewer', __('Reviewers'), ['class' => ' form-label']) }}
            <select name="reviewer[]" class="select2" id="reviewer" multiple>
                @foreach ($reviewer as $id => $name)
                    <option @if ($employeereview->reviewers->contains('user_id', $id)) selected @endif value="{{ $id }}">
                        {{ $name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-12">
            ...
            <select name="status" class="form-control  select2" id="status">
                <option value="running" @if ($employeereview->status == 'Running') selected @endif>{{ __('Running') }}</option>
                <option value="completed" @if ($employeereview->status == 'Completed') selected @endif>{{ __('Completed') }}</option>
                <option value="pending" @if ($employeereview->status == 'Pending') selected @endif>{{ __('Pending') }}</option>
            </select>
        </div>
